...
class UserApiController with Helper{
  ...
  Future<bool> register({
    ...
    if (response.statusCode == 200) {
      Get.snackbar("تمت العملية بنجاح", "تم ارسال كود التحقق الى رقم الموبايل", backgroundColor: kPrimaryColor);
      return true;
    }
...
//1341
  ...
  print(jsonDecode(response.body)['error']);
      Get.snackbar("نجاح", "تم تسجيل الدخول بنجاح" , backgroundColor: kPrimaryColor);
  ...
  Future<bool> resetPassword(BuildContext context,{required String oldPassword,required String newPassword}) async{
    ...
    print(response.statusCode);
    if(response.statusCode == 200){
      // Get.snackbar("نجاح", "تم تغيير كلمة السر بنجاح" , backgroundColor: kPrimaryColor);
